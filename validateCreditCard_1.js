/**
 * @Studio
 * @property {string} studio_numero_cartao - Número do cartão de crédito
 * @property {boolean} studio_formatar - Se true, retorna número formatado em grupos
 */
function ValidarCartaoCredito() {
  const limpo = studio_numero_cartao.replace(/\D/g, '');
  const bandeiras = [
    { nome: 'Visa',       regex: /^4/ },
  ...
  let pos = 0;
  const formatado = studio_formatar
